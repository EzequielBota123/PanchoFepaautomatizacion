...
export async function POST(req: NextRequest) {
  try {
    ...
    const filas: FilaImportFactura[] = rawRows.map((row, idx) => {
      ...
      const totalStr   = col(row, 'total', 'monto', 'importe', 'precio', 'valor')
      const totalRaw   = parseAmount(totalStr)
      const fechaStr   = col(row, 'fecha', 'fechaemision', 'dia') || today
      const fechaVtoStr = col(row, 'fechavto', 'vencimiento', 'fechavencimiento', 'vto', 'fechadevencimiento')
      const metodoStr  = col(row, 'metodopago', 'metodo', 'formapago', 'pago', 'condicionpago') || 'contado'
      ...
      return {
        ...
        tipo,
        total: totalRaw,
        fecha: parseExcelDate(fechaStr, today),
        fecha_vto: fechaVtoStr ? parseExcelDate(fechaVtoStr, '') : '',
        metodo_pago,
        obs,
        errores,
    ...
    if (nuevosNombres.length > 0) {
      const { data: creados } = await sb()
        .from('clientes')
        .insert(nuevosNombres.map(n => ({
          razon_social: validas.find(f => f.cliente_nombre.toLowerCase().trim() === n)!.cliente_nombre,
          activo: true,
        })))
        .select('id,razon_social')
      for (const c of (creados || [])) {
        mapaClientes.set(c.razon_social.toLowerCase().trim(), { id: c.id, saldo_deudor: 0 })
      }
    }

    const { count: facCount } = await sb().from('facturas').select('*', { count: 'exact', head: true })
    ...
    for (const f of (insertadas || [])) {
      if (f.cliente_id) saldoInc.set(f.cliente_id, (saldoInc.get(f.cliente_id) ?? 0) + Number(f.total))
    }
    for (const [cid, inc] of saldoInc.entries()) {
      const cli = (clientesDB || []).find((c: { id: number }) => c.id === cid)
      await sb().from('clientes').update({ saldo_deudor: Number(cli?.saldo_deudor ?? 0) + inc }).eq('id', cid)
    }
